...
namespace uiucprescon {
    namespace ocr {
        ...
        Reader2::Reader2(std::shared_ptr<OCRApi> api) : m_api(api) { m_api->SetPageSegMode(tesseract::PSM_AUTO_OSD); }
        ...
        std::string Reader2::get_ocr_from_image(const std::shared_ptr<Image>& image) const {
            ...
            auto result =
                std::string(std::unique_ptr<char[]>(m_api->get_utf8_text(), std::default_delete<char[]>()).get());
