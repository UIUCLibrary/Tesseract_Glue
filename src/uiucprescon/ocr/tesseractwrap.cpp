...
PYBIND11_MODULE(tesseractwrap, mod, py::mod_gil_not_used()) {
    py::options options;
    py::register_exception<glue::TesseractGlueException>(mod, "TesseractGlueException");
    options.enable_function_signatures();
    mod.doc() = R"pbdoc(Wrapper to Tesseract's C++ API)pbdoc";

    // the classes need to be declared before the functions, otherwise you get a generic_type is already registered
    // error
    py::class_<ocr::Image, std::shared_ptr<ocr::Image>>(mod, "Image", py::module_local())
        .def_property_readonly("w", &ocr::Image::get_w)
        .def_property_readonly("h", &ocr::Image::get_h);
    py::class_<ocr::OCRApi, std::shared_ptr<ocr::OCRApi>>(mod, "OCRApi", "OCR Api for tesseract API.")
    ...
    py::class_<ocr::Reader2>(mod, "Reader")
