...
PYBIND11_MODULE(tesseractwrap, m, py::mod_gil_not_used()) {
    ...
    py::class_<ocr::PDFBuilder>(m, "PDFBuilder", "PDFBuilder builder class.")
        .def(
            py::init([](const std::string &filename, const std::shared_ptr<ocr::OCRApi> &api, const std::string &title) {
                return ocr::PDFBuilder(filename, api, title);
            }),
            py::arg("filename"), py::arg("api"), py::arg("title") = std::string("")
        )
        .def(
            "open",
            [](ocr::PDFBuilder &self){glue::pdf_builder_open(self);},
            "Open pdf file for write."
        )
        .def("close", &ocr::PDFBuilder::close, "Close open file.")
        .def(
            "add_page",
            [](ocr::PDFBuilder &self, const std::string &file_path){
               glue::pdf_builder_add_pages(self, file_path);
            },
            py::arg("file_path"),
            "Add image to pdf"
        )
        ...
        .def(
            "__exit__",
            [](ocr::PDFBuilder &self, const py::object& /*exc_type*/, const py::object& /*exc_value*/, const py::object& /*traceback*/) {
            self.close();
        })
        .def(
            "__enter__",
    ...
    m.def("get_image_lib_versions", &ocr::Capabilities::ImagelibVersions, "Get the version of image libraries being used");
