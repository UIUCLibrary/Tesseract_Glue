...
//                            publishHTML([allowMissing: true, alwaysLinkToLastBuild: false, keepAll: false, reportDir: "${REPORT_DIR}/coverage", reportFiles: 'index.html', reportName: 'Coverage', reportTitles: ''])
                ...
                stage("Run Pytest Unit Tests"){
                    ...
                    steps{
                        dir("build\\lib"){
                            bat "${WORKSPACE}\\venv\\Scripts\\py.test -s --junitxml=${WORKSPACE}/reports/pytest/${junit_filename} --junit-prefix=${env.NODE_NAME}-pytest --cov-report html:${WORKSPACE}/reports/pytestcoverage/ --cov=ocr --integration"
                        }
                    }
                    post {
                        always {
...
//                TODO: pybind11 uses too long of file path
                    bat "pipenv run python setup.py build -t ${WORKSPACE}\\build\\temp\\ sdist -d ${WORKSPACE}\\dist bdist_wheel -d ..\\dist -b ${WORKSPACE}\\build"
                }
